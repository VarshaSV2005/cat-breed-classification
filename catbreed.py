...
st.markdown("### Upload an image from your computer OR paste an image URL")
...
if st.button("Classify the image"):
    if uploaded_file is not None:
        img = uploaded_file  # Use the uploaded image
    elif image_url:
        img = BytesIO(urlopen(image_url).read())  # Use the image URL
    else:
        st.error("Please upload an image or paste a valid URL.")
        st.stop()
